...
public class processing_plotter extends PApplet {
...
public void draw () {
  ...
  for(int i = start; i < dataSize; i++) {
    ...
    for(int j = 1; j < MAX_COLUMNS; j++) {
      ...
      line(
        i - scroll,
        PApplet.parseInt(map(PApplet.parseFloat(thisData[j]), 32767, -32768, 10, height - 20)),
        i - scroll,
        PApplet.parseInt(map(PApplet.parseFloat(nextData[j]), 32767, -32768, 10, height - 20))
      );
    }
  }
...
public void serialEvent (Serial myPort) {
  ...
  for(int i = 0; i < words.length; i++) {
    ...
    float floated = PApplet.parseFloat(trimmed);
    if(Float.isNaN(floated)) {
      ...
      values[a] = PApplet.parseInt(trimmed);
      a++;
    }
  }
...
public void mouseClicked () {
  ...
  public void settings() {  size(800, 400);  noSmooth(); }
  static public void main(String[] passedArgs) {
    String[] appletArgs = new String[] { "processing_plotter" };
    if (passedArgs != null) {
